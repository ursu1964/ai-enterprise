...
SPEC_FILES = ("evidence.v1.json", "objective-optimizer.v1.json", "dashboard.v1.json",
              "cross-domain-reasoning.v1.json", "strategic-memory.v1.json",
              "cognitive-governance.v1.json", "strategic-intelligence.v1.json")
REQUIRED_VIEWS = {
    "strategic-health": {"objective-progress", "investment-alignment", "delivery-confidence"},
    "organizational-intelligence": {"emerging-bottlenecks", "capability-maturity", "collaboration-patterns"},
    "technology-landscape": {"framework-adoption", "technology-lifecycle", "modernization-opportunities"},
    "risk-intelligence": {"projected-architectural-debt", "concentration-risks", "dependency-risks", "organizational-fragility"},
}
...
def verify(root: Path) -> IntelligenceReport:
    ...
    expected_shapes: dict[str, dict[str, type[Any]]] = {
        ...
        "cross-domain-reasoning.v1.json": {"domains": list, "inferences": list, "requirements": dict},
        "strategic-memory.v1.json": {"items": list, "policy": dict},
        "cognitive-governance.v1.json": {"policy": dict},
        "strategic-intelligence.v1.json": {
            "capabilities": list, "recommendations": list, "lifecycle": list,
    ...
    for name, shapes in expected_shapes.items():
        ...
        if invalid_shape:
            findings.append(Finding("intelligence-shape", name, "required object or collection has wrong type"))
    collections = (
        ("evidence.v1.json", "evidence"), ("objective-optimizer.v1.json", "investments"),
        ("dashboard.v1.json", "views"), ("cross-domain-reasoning.v1.json", "inferences"),
    ...
    nested_collections = (
        ...
        ("cross-domain-reasoning.v1.json", "inferences",
         ("source_evidence", "counterevidence", "affected_systems"), False),
        ("strategic-memory.v1.json", "items", ("evidence",), False),
        ("strategic-intelligence.v1.json", "recommendations",
         ("history", "required_investment_ids", "source_inference_ids", "evidence",
        ...
        for parent in parents:
            ...
            for field in fields:
                ...
                if (not isinstance(value, list)
                    ...
                    or (isinstance(value, list) and string_items
                        and any(not isinstance(item, str) for item in value))):
                    findings.append(Finding("intelligence-shape", name,
                                            f"{parent_field}.{field} has wrong type"))
    ...
    if isinstance(recommendations, list) and any(
        ...
        findings.append(Finding("intelligence-shape", "strategic-intelligence.v1.json",
    ...
    if isinstance(inferences, list) and any(
        isinstance(inference, dict) and isinstance(inference.get("source_evidence"), list)
        and any(not isinstance(source, dict) for source in inference["source_evidence"])
        for inference in inferences
    ):
        findings.append(Finding("intelligence-shape", "cross-domain-reasoning.v1.json",
    ...
    if any(item.check == "intelligence-shape" for item in findings):
        ...
        workflow_text = workflow.read_text(encoding="utf-8") if (
            engineering_verify._inside(root, workflow) and workflow.is_file()
            and not workflow.is_symlink()) else ""
        active_lines = {line.strip() for line in workflow_text.splitlines()
                        if line.strip() and not line.lstrip().startswith("#")}
        checks += 1
        if "run: python tools/intelligence_verify.py --json" not in active_lines:
            findings.append(Finding("continuous-intelligence", str(workflow.relative_to(root)),
                                    "strategic intelligence verification absent from CI"))
        canonical = json.dumps({"specifications": specs, "findings": [asdict(item) for item in findings]},
                               sort_keys=True, separators=(",", ":"), ensure_ascii=False)
        return IntelligenceReport(False, checks, hashlib.sha256(canonical.encode()).hexdigest(), (),
                                  tuple(findings))
    ...
    for item in specs.get("evidence.v1.json", {}).get("evidence", []):
        ...
        if (not isinstance(key, str) or key in evidence or not isinstance(digest, str)
    ...
    required_explanation = {"claim", "source_evidence", "reasoning_rule", "confidence",
                            "counterevidence", "affected_systems"}
    prohibited = {"constitutional-amendment", "human-authority-grant", "production-secret-access",
                  "autonomous-funding", "autonomous-deployment"}
    mandatory = {"investment-prioritization", "organizational-restructure", "technology-retirement",
                 "policy-change", "high-risk-architecture"}
    if (evidence.get(str(governance.get("approval_evidence_id"))) != "human-governance-approval"
        ...
        findings.append(Finding("cognitive-governance", "cognitive-governance.v1.json",
    ...
    required_weights = {"expected_value", "confidence", "risk_inverse", "capacity_fit",
                        "constraint_fit"}
    numeric_weights = all(isinstance(value, (int, float)) and not isinstance(value, bool)
                          for value in weights.values())
    optimizer_fields = {"specification_id", "version", "status", "objective_evidence_id",
                        "capacity_evidence_id", "dependency_evidence_id", "capacity_budget",
                        "dependency_declarations_complete", "weights", "investments", "authority"}
    if (set(optimizer) != optimizer_fields or set(weights) != required_weights or not numeric_weights
        or not math.isclose(sum(weights.values()) if numeric_weights else -1, 1.0,
        ...
        findings.append(Finding("objective-optimizer", "objective-optimizer.v1.json",
    ...
    for item in investments:
        checks += 1
        values = [item.get("expected_value"), item.get("risk"), item.get("required_capacity"),
                  item.get("available_capacity"), item.get("constraint_fit")]
        investment_fields = {"investment_id", "expected_value", "confidence", "risk",
                             "required_capacity", "available_capacity", "constraint_fit",
                             "dependencies", "affected_systems", "status"}
        if (set(item) != investment_fields or len(investment_ids) != len(investments)
            or set(item.get("dependencies", []))
            - investment_ids or any(not isinstance(value, (int, float)) or isinstance(value, bool)
            or value < 0 for value in values) or any(item.get(field, 101) > 100
            for field in ("expected_value", "risk", "constraint_fit"))
            ...
            findings.append(Finding("investment", str(item.get("investment_id")),
                                    "duplicate, infeasible, dangling, or pre-decided investment"))
            continue
        score = (weights["expected_value"] * item["expected_value"] / 100
        ...
        ranking.append({"investment_id": item["investment_id"], "score": round(score, 8),
    ...
    if optimizer.get("authority") != {"recommendations_only": True,
        "final_prioritization": "external-human-governance", "automatic_funding": False,
        "automatic_execution": False}:
        findings.append(Finding("optimizer-authority", "objective-optimizer.v1.json",
    ...
    for view in dashboard.get("views", []):
        ...
        checks += len(metrics)
        if (len(metrics) != len(REQUIRED_VIEWS.get(view.get("view_id"), set()))
            or {item.get("metric_id") for item in metrics}
            != REQUIRED_VIEWS.get(view.get("view_id"), set())
            or any(set(item) != {"metric_id", "evidence_id", "confidence", "counterevidence",
                                 "classification"}
                   ...
                   for item in metrics)):
            findings.append(Finding("dashboard", str(view.get("view_id")),
                                    "metric set or provenance incomplete"))
    if dashboard.get("requirements") != {"provenance_per_metric": True, "confidence_visible": True,
        "counterevidence_visible": True, "classification_filtering": True,
        "authorization_before_aggregation": True, "recommendations_not_decisions": True}:
        findings.append(Finding("dashboard-governance", "dashboard.v1.json",
    ...
    for inference in reasoning.get("inferences", []):
        ...
        inference_fields = {"inference_id", "claim", "source_evidence", "reasoning_rule",
                            "confidence", "counterevidence", "causality_claimed", "affected_systems",
                            "status"}
        if (set(inference) != inference_fields or inference.get("inference_id") in inference_ids or len(sources)
            < policy.get("minimum_evidence_sources", 999) or len(domains)
            < policy.get("minimum_distinct_domains", 999) or set(evidence_ids) - set(evidence)
            or len(evidence_ids) != len(set(evidence_ids)) or not domains <= allowed_domains
            or not policy.get("confidence_minimum", 1) <= inference.get("confidence", -1)
            <= policy.get("confidence_maximum", 0) or not inference.get("counterevidence")
            ...
            findings.append(Finding("cross-domain-reasoning", str(inference.get("inference_id")),
    ...
    for item in memory_items:
        ...
        expected_hash = hashlib.sha256(json.dumps(hash_payload, sort_keys=True,
        ...
        try:
            ...
            dates_valid = False
        memory_fields = {"memory_id", "memory_type", "statement", "recorded_at", "valid_from",
                         "valid_until", "temporal_status", "evidence", "classification",
                         "memory_hash", "supersedes"}
        if (set(item) != memory_fields or len(memory_ids) != len(memory_items)
            or not isinstance(digest, str) or digest != expected_hash
            or digest in memory_hashes or not item.get("evidence")
            or set(item.get("evidence", [])) - set(evidence)
            or item.get("temporal_status") not in {"current", "stale", "superseded", "expired",
                                                    "withdrawn", "disputed"}
            or item.get("supersedes") == item.get("memory_id")
            or item.get("supersedes") not in memory_ids | {None} or not dates_valid):
            findings.append(Finding("strategic-memory", str(item.get("memory_id")),
                                    "identity, evidence, temporal state, hash, or supersession invalid"))
        if isinstance(digest, str):
            memory_hashes.add(digest)
    if memory.get("policy") != {"immutable_items": True, "versioned_corrections": True,
        "evidence_required": True, "contradictions_explicit": True, "expiry_evaluated": True,
        "raw_reasoning_forbidden": True, "retention_years": 10}:
        findings.append(Finding("strategic-memory-policy", "strategic-memory.v1.json",
    ...
    required_capabilities = {"strategic-recommendations", "investment-analysis",
        "enterprise-simulations", "organizational-diagnostics", "long-term-forecasting",
        "capability-roadmaps", "executive-decision-support"}
    if set(intelligence.get("capabilities", [])) != required_capabilities:
        findings.append(Finding("intelligence-layer", "strategic-intelligence.v1.json",
    ...
    for recommendation in intelligence.get("recommendations", []):
        ...
        portfolio_capacity = sum(item.get("required_capacity", 0) for item in investments
                                 if item.get("investment_id") in required_ids)
        recommendation_fields = {"recommendation_id", "category", "objective", "expected_benefit",
                                 "confidence", "risk", "affected_systems", "required_investment_ids",
                                 "source_inference_ids", "evidence", "proposed_by", "status", "history",
                                 "review", "authority"}
        if (set(recommendation) != recommendation_fields or set(review) != {
            "actor_type", "actor_id", "evidence_id"
        } or not history or recommendation.get("status") != history[-1]
            or len(indexes) != len(history) or any(second != first + 1 for first, second in pairwise(indexes))
            or len(required_ids) != len(set(required_ids)) or set(required_ids) - investment_ids
            ...
            findings.append(Finding("strategic-recommendation",
                                    str(recommendation.get("recommendation_id")),
                                    "lineage, confidence, evidence, review, or authority invalid"))
    if intelligence.get("layer_boundaries") != {"operational_layer": "executes-approved-work",
        "governance_layer": "determines-permitted-actions", "cognitive_layer": "proposes-considerations",
        "cognitive_execution_authority": False, "cognitive_approval_authority": False}:
        findings.append(Finding("layer-boundary", "strategic-intelligence.v1.json",
    ...
    if "run: python tools/intelligence_verify.py --json" not in lines:
        findings.append(Finding("continuous-intelligence", str(workflow.relative_to(root)),
                                "strategic intelligence verification absent from CI"))
    canonical = json.dumps({"specifications": specs, "investment_ranking": ranking,
        "ci_workflow_hash": hashlib.sha256(workflow_text.encode()).hexdigest(),
        "findings": [asdict(item) for item in findings]}, sort_keys=True,
        separators=(",", ":"), ensure_ascii=False)
    return IntelligenceReport(not findings, checks, hashlib.sha256(canonical.encode()).hexdigest(),
                              tuple(ranking), tuple(findings))
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps(asdict(report), sort_keys=True) if args.as_json else
          f"P16 governed intelligence: {'PASS' if report.conformant else 'FAIL'} "
